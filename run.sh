...
docker exec -i $DOCKER_ID /bin/bash -c 'supervisord -c /etc/supervisor/conf.d/qsluck.conf'
#docker exec -i $DOCKER_ID /bin/bash -c 'supervisord -c /etc/supervisor/conf.d/lucksocket.conf'
